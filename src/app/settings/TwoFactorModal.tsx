...
export default function TwoFactorModal({
  ...
  async function removeTwofa() {
    const res = await fetch("/api/2fa/remove2fa", {
      method: "DELETE",
      body: JSON.stringify({
        token: twoFactorCode,
      }),
    ...
    if (!data.ok) {
      setError({ show: true, message: data.message, type: false });
      return;
    }

    setError({
      show: true,
      message: "Success! Two Factor Authentication is disabled.",
      type: true,
    });

    showTwoFactorModal({ show: true, refresh: true });
  }
  ...
  useEffect(() => {
    ...
    if (session && twoFactorModal.show && !user?.twoFactorAuth)
      getTwoFactorCode();
  }, [twoFactorModal.show]);
              ...
              <div>
                <div className="space-y-2 p-4 mt-3 text-[15.5px] leading-relaxed text-gray-500">
                  {!user?.twoFactorAuth && !verified ? (
                    <>
                      ...
                      <p className="text-gray-200 text-[10px] md:text-sm">
                    ...
                    <>
                      <h1 className="text-gray-200 text-lg mb-10">
                        You are already protected with 2fa!
                      </h1>
                      <form>
                        <label className="block text-md font-medium text-gray-200 dark:text-textDarkmode">
                          Enter a code from your authenticator app to remove 2fa
                          <span className="text-red-500">*</span>
                        </label>
                        <input
                          required
                          className="h-auto block px-3 py-2 text-gray-900   border rounded-md shadow-md focus:ring-blue-500 focus:border-blue-500 sm:text-md"
                          placeholder="Code"
                          onChange={(e) => setTwoFactorCode(e.target.value)}
                          type="text"
                        />
                        <button
                          className="mt-3 px-3 py-1 text-white bg-blue-500 rounded-md outline-none ring-offset-2 ring-blue-400 focus:ring-2"
                          disabled={false}
                          type="submit"
                          onClick={(e) => {
                            e.preventDefault();
                ...
                <div className="flex items-center gap-3 p-4 mt-5 border-t">
                  ...
                  <button
                    ...
                    onClick={() => showTwoFactorModal({ show: false })}
                  >
                    {user?.twoFactorAuth || verified ? "Close" : "Cancel"}
                  </button>
                </div>
              </div>
            </div>
